...
class MetaWorldEnv(MetaEnvironment):
    def __init__(self, *args, **kwargs):
        ...
        if ml10or45 == 10:
            ...
            num_grid_tasks_per_base_task = int(kwargs['n_grid_tasks'] / 15)
        elif ml10or45 == 45:
            self.ml_env = metaworld.ML45()
            num_train_tasks_per_base_task = int(kwargs['n_train_tasks'] / 45)
            num_test_tasks_per_base_task = int(kwargs['n_eval_tasks'] / 5)
            num_grid_tasks_per_base_task = int(kwargs['n_grid_tasks'] / 50)
        elif ml10or45 == 1:
            ...
            num_grid_tasks_per_base_task = int(kwargs['n_grid_tasks'] / 2)
        elif ml10or45 == 3:
            self.ml_env = mw_bench.ML3()
            num_train_tasks_per_base_task = int(kwargs['n_train_tasks'] / 3)
            num_test_tasks_per_base_task = int(kwargs['n_eval_tasks'])
            num_grid_tasks_per_base_task = int(kwargs['n_grid_tasks'] / 4)
        else:
        ...
        self.sample_tasks(num_train_tasks_per_base_task, num_test_tasks_per_base_task, num_grid_tasks_per_base_task)

    def sample_tasks(self, num_train_tasks_per_base_task, num_test_tasks_per_base_task, num_grid_tasks_per_base_task):
        self.grid_tasks = []
        self.train_tasks = []
        for name, env_cls in self.ml_env.train_classes.items():
            ...
            tasks = random.sample([task for task in self.ml_env.train_tasks if task.env_name == name],
                                  num_grid_tasks_per_base_task)
            self.grid_tasks += tasks
        ...
        for name, env_cls in self.ml_env.test_classes.items():
            ...
            tasks = random.sample([task for task in self.ml_env.test_tasks if task.env_name == name],
                                  num_grid_tasks_per_base_task)
            self.grid_tasks += tasks
    ...
    def step(self, action):
        ...
        info['true_task'] = dict(base_task=self.name2number[self.active_env_name], specification=self.metaworld_env._target_pos, target=self.metaworld_env._target_pos)

        if self.active_env_name == "reach-v2":
            info['achieved_spec'] = self.metaworld_env.tcp_center
        elif self.active_env_name == "reach-v1":
            rightFinger, leftFinger = self.metaworld_env._get_site_pos('rightEndEffector'),\
                                      self.metaworld_env._get_site_pos('leftEndEffector')
            info['achieved_spec'] = (rightFinger + leftFinger) / 2
        else:
            # TODO implement for all used environments
            pass
