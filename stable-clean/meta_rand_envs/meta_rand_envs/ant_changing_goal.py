...
class AntChangingGoalEnv(RandomEnv, utils.EzPickle):
    ...
    def _step(self, action):
        ...
        return ob, reward, done, dict(
            ...
            achieved_spec=xposafter[:2])

    # from pearl rlkit
    def _get_obs(self):
        """
        Like https://github.com/openai/gym/blob/master/gym/envs/mujoco/ant_v3.py
        with exclude_current_positions_from_observation=False
        """
        return np.concatenate([
            self.sim.data.qpos.flat,
            self.sim.data.qvel.flat,
            # self.get_body_com("torso").flat, doesn't seem to be required, also not in gym observation
    ...
    def generate_grid_tasks(self, num_tasks):
        a = 2 * np.pi * np.arange(num_tasks + 1) / num_tasks
        r = self.task_max_radius * (np.arange(num_tasks + 1) / num_tasks) ** 0.5
        goals = np.stack((r * np.cos(a), r * np.sin(a)), axis=-1)
        tasks = [{'goal': goal, 'angle': angle, 'radius': rv} for goal, angle, rv in zip(goals, a, r)]
        return tasks
