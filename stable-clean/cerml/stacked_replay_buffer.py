...
class StackedReplayBuffer:
    ...
    def add_episode(self, episode, task_nr=None):
        ...
        if self._rewards_elbo is not None:
            self._rewards_elbo[indices_list] = episode['rewards_elbo']
    ...
    def sample_few_step_batch(self, points, batch_size, normalize=True, whole_episode=False):
        ...
        if not whole_episode:
            all_indices = points[:, np.newaxis] + np.arange(-self.time_steps, 1)[np.newaxis, :]
        else:
            all_indices = points[:, np.newaxis] + np.arange(-self.max_path_length, self.max_path_length)[np.newaxis, :]
    ...
    def sample_random_few_step_batch(self, points, batch_size, normalize=True, prio=None, return_sac_data=False,
                                     return_whole_episodes=False):
        ''' batch of unordered small sequences of transitions '''
        indices = self.get_indices(points, batch_size, prio=prio)
        enc_data = self.sample_few_step_batch(indices, batch_size, normalize=normalize)
        if return_sac_data:
            return enc_data, self.sample_data(indices)
        elif return_whole_episodes:
            # efficiency could be increased by only sampling (and e.g. normalizing) once and just returning the full
            # episode at the appropriate indices as enc_data
            return enc_data, self.sample_few_step_batch(indices, batch_size, normalize=normalize, whole_episode=True)
        else:
            return enc_data
    ...
    def unnormalize_data(self, data):
        for key in data.keys():
            if key in self.stats_dict:
                data[key] = data[key] * (self.stats_dict[key]["std"] + 1e-8) + self.stats_dict[key]["mean"]
        return data
    ...
    def check_enc(self):
        ...
        for base in base_spec_dict.keys():
            ...
            for i, spec in enumerate(base_spec_dict[base]):
                spec_map = spec_tasks_array == spec
                if spec_map.ndim == 2:
                    spec_map = spec_map.all(axis=1)
                task_indices = np.where(np.logical_and(base_tasks_array == base, spec_map))[0]
