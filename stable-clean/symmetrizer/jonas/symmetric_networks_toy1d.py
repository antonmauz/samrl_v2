...
from symmetrizer.symmetrizer.ops import c2g
...
class Toy1DPolicy(torch.nn.Module):
    ...
    def forward(self, obs):
        """
        obs: (state [batch_size, 1], latent grid [batch_size, 1 or num_channels, 1, width])
        (1 if only using most likely channel)
        """
...
class Toy1DQNet(torch.nn.Module):
    ...
    def __init__(self, input_size, hidden_sizes=[256, 128], channels=[16],
        ...
        between_group = C1Intermediate()
        fcs = [BasisLinear(input_size + y_prep_size * 2, hidden_sizes[0], between_group, gain_type=gain_type,
                           basis=basis, bias_init=True)]
        for i in range(len(hidden_sizes) - 1):
            fcs.append(BasisLinear(hidden_sizes[i], hidden_sizes[i + 1], between_group, gain_type=gain_type,
                                   basis=basis, bias_init=True))
    ...
    def forward(self, obs, action):
        """
        obs: (state [batch_size, 1], latent grid [batch_size, 1, width])
        action: [batch_size, 1]
        """
        coordinates, grid = obs
        for i, c in enumerate(self.convs):
        ...
        fc_output = torch.cat((pool, prep_obs, prep_act), dim=1)
        for fc in self.fcs:
            fc_output = F.relu(fc(fc_output))
