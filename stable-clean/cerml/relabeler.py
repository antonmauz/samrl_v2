    ...
    def relabel(self):
        ...
        while start < total_points:
            ...
            if self.latent_probabilities:
                y_distr, z_distrs = self.encoder.encode(encoder_input)
                z = torch.cat([y_distr.probs] + [torch.cat((d.mean, d.variance), dim=1) for d in z_distrs], dim=1)
                y = torch.argmax(y_distr.probs, dim=1)
            else:
