...
class StackedReplayBuffer:
    def __init__(self, max_replay_buffer_size, time_steps, observation_dim, action_dim, task_indicator_dim, data_usage_reconstruction, data_usage_sac, num_last_samples, permute_samples, encoding_mode, use_rewards_elbo):
        ...
        if use_rewards_elbo:
            self._rewards_elbo = np.zeros((max_replay_buffer_size, 1), dtype=np.float32)
        else:
            self._rewards_elbo = None
    ...
    def add_episode(self, episode, task_nr=None):
        ...
        if self._rewards_elbo is not None:
            self._rewards_elbo[low:high] = episode['rewards_elbo']
        ...
        # TODO activate if task_info_dict is needed
    ...
    def add_sample(self, observation, action, reward, reward_elbo, terminal,
        ...
        if self._rewards_elbo is not None:
            self._rewards_elbo[self._top] = reward_elbo
    ...
    def sample_data(self, indices):
        ...
        if self._rewards_elbo is not None:
            ret['rewards_elbo'] = self._rewards_elbo[indices]
        return ret
    ...
    def sample_few_step_batch(self, points, batch_size, normalize=True, limit_time_steps=True):
    ...
    def sample_random_few_step_batch(self, points, batch_size, normalize=True, limit_time_steps=True):
        ...
        if step_mode == 'decoder' and self.decoder_full_episode_window:
            ...
            if np.sum(~padding_mask) != (batch_size * self.max_path_length):
                raise ValueError('Not all sample episodes had the same length. Cannot reconstruct whole episode.')
            else:
        ...
        return self.sample_few_step_batch(indices, batch_size, normalize=normalize, limit_time_steps=limit_time_steps)
    ...
    def check_enc(self):
        ...
        try:
            ...
            for sub in true_task_list:
                if not isinstance(sub, dict) or 'base_task' not in 'sub':
                    print(sub)
                print(e)
        base_spec_dict = {}
        for base_task in base_tasks:
            spec_list = list(set([robust_tuple(sub['specification']) for sub in true_task_list if sub['base_task'] == base_task]))
        ...
        for base in base_spec_dict.keys():
            ...
            for i, spec in enumerate(base_spec_dict[base]):
                task_indices = [index for index in indices if (self._true_task[index][0]['base_task'] == base and robust_tuple(self._true_task[index][0]['specification']) == spec)]
                target = None
