...
class RolloutCoordinator:
    def __init__(self,
                 ...
                 gpu_id,
                 scripted_policy,
                 exploration_elbo_reward_factor
                 ):
        self.env = env
    ...
    def collect_data(self, tasks, train_test, deterministic=False, num_samples_per_task=np.inf,
                     num_trajs_per_task=np.inf, animated=False, save_frames=False, task_specs_to_policy=False):
        ...
        while n_steps_total < num_samples_per_task * len(tasks) and n_trajs < num_trajs_per_task * len(tasks):
            for i in range(num_max_batches):
                ...
                if self.use_multiprocessing:
                    assert sum(ray.get([worker.reset_buffer.remote(temp_tasks) for worker, temp_tasks in
                                        zip(self.workers, active_tasks)])) == num_tasks
                else:
    ...
    def rollout(self, n_tasks, deterministic, animated, save_frames, task_specs_to_policy):
        ...
        paths = [dict(
            ...
            actions=[],
            rewards=[],
            rewards_elbo=[], # some empty lists shouldn't be a problem
            next_observations=[],
        ...
        while path_length < self.max_path_length:
            ...
            if task_specs_to_policy and true_tasks is not None:
                z_debug = ptu.from_numpy(np.array([t["specification"] for t in true_tasks]))
                if z_debug.ndim == 1:
                    z_debug = z_debug[:, None]
                y_debug = ptu.from_numpy(np.array([t["base_task"] for t in true_tasks]))
                spec_debug = z_debug, y_debug
            # Step
            ...
            for active_path_nr, path_nr in enumerate(terminals_map):
                ...
                assert ('base_task' in true_tasks[active_path_nr])  # To find the bug in check_encoding
                paths[path_nr]['true_tasks'].append(true_tasks[active_path_nr])

            # TODO is it correct/enough to just do this after appending?
            if self.exploration_elbo_reward_factor != 0.0:
                if path_length < self.time_steps:
                    # TODO pad input with zeros instead of leaving out elbo loss for first 10 steps?
                    rewards_elbo = np.zeros((n_tasks, 1))
                else:
                    with torch.no_grad():
                        reconst_data = dict(
                            observations=np.stack([np.stack(
                                paths[active_path_i]['observations'][path_length - self.time_steps:path_length + 1]) for
                                                   active_path_i in terminals_map]),
                            next_observations=np.stack([np.stack(
                                paths[active_path_i]['next_observations'][path_length - self.time_steps:path_length + 1])
                                                        for active_path_i in terminals_map]),
                            actions=np.stack(
                                [np.stack(paths[active_path_i]['actions'][path_length - self.time_steps:path_length + 1])
                                 for active_path_i in terminals_map]),
                            rewards=np.stack(
                                [np.stack(paths[active_path_i]['rewards'][path_length - self.time_steps:path_length + 1])
                                 for active_path_i in terminals_map]),
                            task_indicators=np.stack([np.stack(
                                paths[active_path_i]['task_indicators'][path_length - self.time_steps:path_length + 1]) for
                                                      active_path_i in terminals_map]),
                            next_task_indicators=np.stack([np.stack(
                                paths[active_path_i]['next_task_indicators'][path_length - self.time_steps:path_length + 1])
                                                           for active_path_i in terminals_map]),
                            terminals=np.stack([np.stack(
                                paths[active_path_i]['terminals'][path_length - self.time_steps:path_length + 1])[:, None]
                                                for active_path_i in terminals_map]),
                            true_tasks=np.stack([np.stack(
                                paths[active_path_i]['true_tasks'][path_length - self.time_steps:path_length + 1])[:, None]
                                                 for active_path_i in terminals_map])
                        )
                        _, rewards_elbo, _, _, _, _ = self.reconstruction_trainer.calculate_elbo(reconst_data,
                                                                                              len(terminals_map))
                        rewards_elbo = ptu.get_numpy(rewards_elbo)[:, None]

                for active_path_nr, path_nr in enumerate(terminals_map):
                    paths[path_nr]['rewards_elbo'].append(rewards_elbo[active_path_nr].copy())
        ...
        for path in paths:
            ...
            results.append({key: (
                np.expand_dims(np.array(path[key]), axis=1) if key in ['terminals', 'true_tasks'] else np.array(
                    path[key])) for key in path.keys()})
    ...
    def step_workers(self, actions, terminals, animated, save_frames):
        ...
        if self.use_multiprocessing:
            out_ = ray.get([worker.step_envs.remote(actions[tasks] if actions is not None else None,
                                                    terminals[tasks] if terminals is not None else None, animated,
                                                    save_frames) for tasks, worker in
                            zip(self.index_table, self.workers) if len(tasks) > 0])
        else:
    ...
    def collect_replay_data(self, tasks, task_specs_to_policy, num_samples_per_task=np.inf):
        num_env_steps = 0
        results = self.collect_data(tasks, 'train', deterministic=False, num_samples_per_task=num_samples_per_task,
                                    animated=False, task_specs_to_policy=task_specs_to_policy)
        for t_ind, path in enumerate(results):
            self.replay_buffer.add_episode(path, task_nr=tasks[t_ind % len(tasks)])
    ...
    def evaluate(self, train_test, tasks, num_eval_trajectories, deterministic=True, animated=False, save_frames=False):
        results = self.collect_data(tasks, train_test, deterministic=deterministic,
                                    num_trajs_per_task=num_eval_trajectories, animated=animated,
        ...
        if self.exploration_elbo_reward_factor != 0.0:
            per_path_rewards_elbo = np.array([np.sum(path['rewards_elbo']) for path in results])
            eval_average_reward_elbo = per_path_rewards_elbo.mean()
            eval_std_reward_elbo = per_path_rewards_elbo.std()
            eval_max_reward_elbo = per_path_rewards_elbo.max()
            eval_min_reward_elbo = per_path_rewards_elbo.min()

            eval_statistics[train_test + '_eval_avg_reward_elbo' + deterministic_string] = eval_average_reward_elbo
            eval_statistics[train_test + '_eval_std_reward_elbo' + deterministic_string] = eval_std_reward_elbo
            eval_statistics[train_test + '_eval_max_reward_elbo' + deterministic_string] = eval_max_reward_elbo
            eval_statistics[train_test + '_eval_min_reward_elbo' + deterministic_string] = eval_min_reward_elbo

        # New: eval statistics per task
        eval_statistics[train_test + '_eval_per_task' + deterministic_string] = {
            i: np.mean([np.sum(path['rewards']) for path in results if path['true_tasks'][0, 0]['base_task'] == i]) for
            i in np.unique([path['true_tasks'][0, 0]['base_task'] for path in results])
        }

        # success rates for meta world
        if 'success' in results[0]['env_infos'][0]:
            success_values = np.array(
                [int(any([timestep['success'] for timestep in path['env_infos']])) for path in results])
            success_rate = np.sum(success_values) / success_values.shape[0]
            ...
            # for base_task in np.unique(base_tasks):
            #     print(base_task, success_values[base_tasks == base_task])
            success_rates_per_base_task = {base_task: success_values[base_tasks == base_task].mean() for base_task in
                                           np.unique(base_tasks)}
...
class RolloutWorker:
    ...
    def reset_buffer(self, active_task_list):
        ...
        for ind, t in enumerate(self.active_task_list):
            if t in task_duplicate_dict:
                ...
                self.active_task_list[ind] = t + int(
                    num_divisor * (1. / num_divisor * task_duplicate_dict[t])) / num_divisor
            else:
    ...
    def step_envs(self, actions, terminals, animated=False, save_frames=False):
        ...
        for i, task_nr in enumerate([self.active_task_list[t_] for t_ in np.where(terminals == False)[
            0]] if terminals is not None else self.active_task_list):
            ...
            if actions is None:
                ...
                if save_frames:
                    image = np.flipud(self.env.get_image(width=640, height=480))
