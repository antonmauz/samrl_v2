...
from symmetrizer.symmetrizer.groups.groups import MatrixRepresentation
from symmetrizer.symmetrizer.ops.ops import get_basis, get_coeffs, get_invariant_basis, \
    compute_gain
...
class BasisLayer(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        """
        self.W = torch.sum(self.basis*self.coeffs, 0)  # TODO I guess that could be optimized by only recalculating self.W after update of coeffs instead of each time (using hooks)
...
class MultiBatchBasisLinear(BasisLinear):
    """
    BasisLinear that is able to deal with multiple batch dimensions. This isn't incorporated in BasisLinear directly
    as reshaping only the network input and output (e.g. in GRU) might be more efficient than reshaping e.g. each GRU
    timestep
    """
    def forward(self, x):
        old_shape = list(x.shape[:-2])
        res = super().forward(x.reshape(-1, x.shape[-2], x.shape[-1]))
        return res.reshape(old_shape + list(res.shape[-2:]))
